...
void InitMemMgr(void)
{
  ...
  OSMemCreate(&bfrPool, "Buffer Pool", bfrSpace, PoolSize, sizeof(PBuffer), &osErr);
  assert(osErr == OS_ERR_NONE);
}
...
PBuffer *Allocate(void)
{
	OS_ERR		osErr;/* -- uCos Error Code */   
        ...
        PBuffer *bfr;  // Allocated buffer address
	...
	bfr = (PBuffer*) OSMemGet(&bfrPool, &osErr);
	assert(osErr==OS_ERR_NONE);
